...
Supported providers (set per team via `provider:` in config.yaml):
  ...
  - "timetoscore": Black Bear / TimeToScore API (api.blackbear.timetoscore.com).
      api_url should point at the FULL, un-scoped `get_schedule` request
      (no `team_id` param) so opponent-history/head-to-head can see every
      team's games. standings_api_url returns all levels/conferences in
      one call. These endpoints are signed (auth_signature/auth_timestamp)
      and NOT self-generated by this script — when a signed URL expires,
      that team's fetch will fail; the script logs it clearly and leaves
      that team's previously-generated .ics file untouched rather than
      failing the whole run. Grab a fresh signed URL from the league site
      (browser devtools network tab) and update config.yaml when that happens.
...
teams:
  - name: ...
    ...
    slug: ...
    league_name: ...
    api_url: ...                       # bondsports: game-scores; timetoscore: full get_schedule
    standings_api_url: ...             # recommended
...
def main() -> None:
    ...
    # Some team entries (e.g. multiple TimeToScore teams in the same league) share
    # the same full-schedule/standings URL; cache within this run to avoid re-fetching.
    ...
    for team_entry in teams:
        ...
        # Signed provider URLs (timetoscore) can expire; don't let one team's stale
        # signature crash the whole run. Log clearly and leave prior .ics file(s) as-is.
        try:
            raw_games = cached_fetch(games_url)
            if provider == "timetoscore":
                all_games = parse_games_timetoscore(raw_games.get("games", []), game_length_minutes=game_length_minutes)
        ...
        for my_team_id, my_team_name in zip(my_ids, my_names):
            ...
            if standings_url:
                try:
                    standings_raw = cached_fetch(str(standings_url))
                    if provider == "timetoscore":
                    ...
                    print(f"WARNING: {out_file}: failed to fetch/parse standings ({exc}). Continuing without standings this run.")
